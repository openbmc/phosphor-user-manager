...
Config::Config(sdbusplus::bus::bus& bus, const char* path, const char* filePath,
               const char* caCertFile, const char* certFile,ConfigIface::Type lDAPType,
               ConfigMgr& parent) :
    Ifaces(bus, path, true),
    tlsCacertFile(caCertFile), tlsCertFile(certFile),configFilePath(filePath), objectPath(path),
