                                            ...
                                            const fs::path& entryPath)
{
    ...
    {
        lg2::error("Skipping role mapping entry with non-numeric id {ID}: {PATH}",
                   "ID", idStr, "PATH", entryPath);
        return std::nullopt;
    }
    catch (const std::out_of_range&)
    {
        lg2::error("Skipping role mapping entry with out-of-range id {ID}: {PATH}",
                   "ID", idStr, "PATH", entryPath);
        return std::nullopt;
    }
    ...
    {
        lg2::error("Skipping role mapping entry with id exceeding max value {ID}: "
                   "{PATH}",
                   "ID", idStr, "PATH", entryPath);
        return std::nullopt;
    }
...
void Config::restoreRoleMapping()
{
    ...
    for (auto& file : fs::directory_iterator(dir))
    {
        ...
        {
            lg2::error("Skipping non-regular role mapping entry: {PATH}", "PATH",
                       file.path());
            continue;
        }
