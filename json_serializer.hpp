...
class JsonSerializer
{
  public:
    ...
    void erase(Key&& key)
    {
        static_assert(StringType<Key>, "Key must satisfy StringType");
        jsonData.erase(std::forward<Key>(key));
        lg2::info("Erased key {KEY}", "KEY", key);
        lg2::info("Json {DATA}", "DATA", jsonData.dump());
    }
    bool store()
    {
