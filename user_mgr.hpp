...
using Value = std::variant<Privilege, GroupList, UserEnabled>;
using UserInfoMap = std::map<PropertyName, Value>;
...
class UserMgr : public UserMgrIface, AccountPolicyIface
{
  public:
     ...
     * then get the privilege mapping for the LDAP group returns.
     *
     * @param[in] - user name
