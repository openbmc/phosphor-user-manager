...
class UserMgr : public Ifaces
{
  ...
  protected:
    ...
    /** @brief checks if the group exists; throws if it doesn't exist
     * @param groupName - group to check
     */
    void checkAndThrowForGroupExist(const std::string& groupName);

    /** @brief checks if the group doesn't exist; throws if it exists
     * @param groupName - group to check
     */
    void checkAndThrowForGroupNotExist(const std::string& groupName);

    /** @brief checks if the number of group managed by this daemon exceeds the
     * limit
     */
