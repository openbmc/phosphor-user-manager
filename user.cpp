                         ...
                         const std::string& salt)
{
    ...
    if ((shadow)() == NULL)
    {
        raiseException(errno, "Error opening shadow file");
    }
    ...
    if (r < 0)
    {
        raiseException(errno, "Error reading shadow file mode");
    }

    r = fchmod(fileno((temp)()), st.st_mode);
    if (r < 0)
    {
        raiseException(errno, "Error setting temp file mode");
    }
    ...
    while (true)
    {
        ...
        if (r < 0)
        {
            raiseException(errno, "Error updating temp shadow file");
        }
    } // All entries
