...
void UserMgr::initUserObjects(void)
{
    ...
    if (!userNameList.empty())
    {
        ...
        {
            deleteUser(name);
            removeConfKey(bootStrapStateFile, name);
        }
    }
}
...
uint8_t UserMgr::updateBootStrapState(std::string userName, bool isBootStrap)
{
    lg2::error(
        "updateBootStrapState isBootStrap {STATE} for user {NAME} contain {CON}",
        "STATE", isBootStrap, "NAME", userName, "CON",
    ...
    if (setPamModuleConfValue(bootStrapAccountStateFile, userName,
                              std::to_string(isBootStrap), true) != success)
    {
        lg2::error(
            "Failed to backup the BootStrapAccount state {STATE} for user {NAME}",
            "STATE", isBootStrap, "NAME", userName);
        return failure;
    }
