...
enum privMgrLevel {PRIV_ADMIN,PRIV_OPERATOR,PRIV_USER,PRIV_NOACCESS};
    ...
    else
    {
        ...
        try
        {
            ...
            for (const auto& obj : objects)
            {
			    ...
			    // if both groupname and pivilege are set then check if user is part of group
                            if(!groupName.empty() && !privilege.empty())
                            {
                              // check if user if in Group so we can give privilege
                              for(i=0;i<(int)ldapGroupName.size();++i)
                                {
                                  if (groupName == ldapGroupName[i])
                                    {
                                      // check priv level
                                      for(level=0;level<(int)privMgr.size();++level)
                                        {
                                          // update priv level if higher than current level AND if priv is set
                                          if(privilege == privMgr[level] && level <= currentPriv)
                                            {
                                              currentPriv = level;
