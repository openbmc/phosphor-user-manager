...
static constexpr size_t IPMI_MAX_USERS = 15;
static constexpr size_t IPMI_MAX_USER_NAME_LEN = 16;
static constexpr size_t SYSTEM_MAX_USER_NAME_LEN = 30;
static constexpr size_t MAX_SYSTEM_USERS = 30;
...
void UserMgr::checkUserExists(const std::string &userName)
{
    if (userName.empty())
    {
...
void UserMgr::checkUserDoesNotExist(const std::string &userName)
{
    if (userName.empty())
    {
        log<level::ERR>("User name is empty");
...
void UserMgr::checkUserNameConstraints(
    const std::string &userName, const std::vector<std::string> &groupNames)
{
    if (std::find(groupNames.begin(), groupNames.end(), "ipmi") !=
        groupNames.end())
    {
        if (userName.length() > IPMI_MAX_USER_NAME_LEN)
        {
            log<level::INFO>("IPMI user name length limitation",
                             entry("SIZE=%d", userName.length()));
            elog<UserNameGrpPrivFail>(
                xyz::openbmc_project::User::Common::UserNameGrpPrivFail::REASON(
                    "IPMI length"));
        }
    }
    else
    {
        if (userName.length() > SYSTEM_MAX_USER_NAME_LEN)
        {
            log<level::INFO>("User name length limitation",
                             entry("SIZE=%d", userName.length()));
            elog<InvalidArgument>(Argument::ARGUMENT_NAME("User name"),
    ...
    {
        log<level::INFO>("Invalid user name",
                         entry("SIZE=%d", userName.length()));
        elog<InvalidArgument>(Argument::ARGUMENT_NAME("User name"),
...
void UserMgr::checkUserCountLimit(const std::vector<std::string> &groupNames)
{
    if (std::find(groupNames.begin(), groupNames.end(), "ipmi") !=
        groupNames.end())
    {
        if (getIpmiUsersCount() >= IPMI_MAX_USERS)
        {
            log<level::INFO>("IPMI user limit reached");
            elog<NoResource>(
    ...
    {
        if ((usersList.size() - getIpmiUsersCount()) >=
            (MAX_SYSTEM_USERS - IPMI_MAX_USERS))
        {
            log<level::INFO>("Non-ipmi User limit reached");
            elog<NoResource>(
                xyz::openbmc_project::User::Common::NoResource::REASON(
                         ...
                         bool enabled)
{
    ...
    checkUserNameConstraints(userName, groupNames);
    checkUserCountLimit(groupNames);
    if (!priv.empty() &&
    ...
    try
    {
        executeFile("/usr/sbin/useradd", userName.c_str(), "-G",
...
void UserMgr::deleteUser(std::string userName)
{
    ...
    try
    {
        executeFile("/usr/sbin/userdel", userName.c_str());
    }
    catch (...)
    {
        log<level::INFO>("User delete failed",
                         entry("USER_NAME=%s", userName.c_str()));
        elog<InternalFailure>();
    }
...
void UserMgr::renameUser(std::string userName, std::string newUserName)
{
    ...
    phosphor::user::shadow::Lock lock();
    checkUserExists(userName);
    checkUserDoesNotExist(newUserName);
    checkUserNameConstraints(newUserName,
                             usersList[userName].get()->userGroups());
    try
    {
        executeFile("/usr/sbin/usermod", "-l", newUserName.c_str(),
                    userName.c_str());
    }
    catch (...)
    {
        log<level::INFO>("User rename failed",
                         entry("USER_NAME=%s", userName.c_str()));
        elog<InternalFailure>();
    }
    ...
    std::string newUserObj = std::string(USERS_OBJECT_PATH) + "/" + newUserName;
                                  ...
                                  const std::string &priv)
{
    ...
    try
    {
        executeFile("/usr/sbin/usermod", userName.c_str(), "-G",
                    addGroup.c_str(), "-s",
...
void UserMgr::userEnable(const std::string &userName, bool enabled)
{
    ...
    try
    {
        executeFile("/usr/sbin/usermod", userName.c_str(), "-e",
                    (enabled ? "" : "1970-01-02"));
    }
    catch (...)
    {
                                   ...
                                   std::vector<std::string> &sshUsersList)
{
    ...
    phosphor::user::File passwd(PASSWD_FILE_NAME, "r");
    if ((passwd)() == NULL)
    {
...
bool UserMgr::isUserEnabled(const std::string &userName)
{
    ...
    struct spwd spwd;
    struct spwd *spwdPtr = &spwd;
    struct spwd *resultPtr;
    int status = getspnam_r(userName.c_str(), spwdPtr, buffer.data(),
                            buffer.max_size(), &resultPtr);
    if (!status && (spwdPtr == resultPtr))
    {
