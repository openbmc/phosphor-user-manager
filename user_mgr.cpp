...
bool UserMgr::isUserExistSystem(const std::string& userName)
{
    ...
    auto *res = getpwnam(userName.c_str());
    return res != nullptr;
}
                         ...
                         bool enabled)
{
    ...
    catch (const InternalFailure& e)
    {
        if(isUserExistSystem(userName)) {
            lg2::error("User created despite error, attempting to delete user", "USERNAME", userName);
...
void UserMgr::deleteUser(std::string userName)
{
    ...
    {
        if(!isUserExistSystem(userName)) {
            lg2::warning("Delete User '{USERNAME}' failed, and user is no longer present, treating as success",
                         "USERNAME", userName);
        } else {
...
void UserMgr::renameUser(std::string userName, std::string newUserName)
{
    ...
    catch (const InternalFailure& e)
    {
        if(isUserExistSystem(newUserName)) {
            lg2::error("Rename '{USERNAME}' to '{NEWUSERNAME}' partially failed", "USERNAME",
