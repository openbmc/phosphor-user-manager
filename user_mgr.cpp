...
bool UserMgr::isUserExistSystem(const std::string& userName)
{
    ...
    try {
        // getPrimaryGroup checks if the user exists, returns the group ID or throws
        getPrimaryGroup(userName);
    } catch(const UserNameDoesNotExist &e) {
        return false;
    }
                         ...
                         bool enabled)
{
    ...
    catch (const InternalFailure& e)
    {
        ...
        {
            lg2::warning("User created despite error, attempting to delete user",
                       "USERNAME", userName);
