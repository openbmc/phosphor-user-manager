...
static constexpr const char *pwQualityConfigFile = "/etc/security/pwquality.conf";
...
bool UserMgr::userLockedForFailedAttempt(const std::string& userName)
{
    ...
    // /usr/sbin/faillock: Error clearing the tally file for {user}:{output from perror}
    ...
    {
        return true; // User account password is locked out
    }
