...
static constexpr const char *pamPasswdQC = "pam_passwdqc.so";
static constexpr const char *pamPWHistory = "pam_pwhistory.so";
static constexpr const char *minPasswdLenProp = "min";
...
uint8_t UserMgr::minPasswordLength(uint8_t value)
{
    ...
    std::string min_parm = std::to_string(value) + "," +
        std::to_string(value) + "," + std::to_string(value) + "," +
        std::to_string(value) + "," + std::to_string(value);
    if (setPamModuleArgValue(pamPasswdQC, minPasswdLenProp,
                             min_parm) != success)
    {
...
uint32_t UserMgr::accountUnlockTimeout(uint32_t value)
{
    ...
    if (setPamModuleArgValue(pamFaillock, unlockTimeout, std::to_string(value)) !=
        success)
    {
...
bool UserMgr::userLockedForFailedAttempt(const std::string &userName)
{
    ...
    // Emulate the behavior of pam_faillock.so authsucc
    ...
    Ifaces(bus, path, true), bus(bus), path(path)
{
    ...
    unsigned long tmp = 0;
    if (getPamModuleArgValue(pamPasswdQC, minPasswdLenProp, valueStr) !=
        success)
    {
    ...
    else
    {
        try
        {
            ...
            // Ignore the remainder of the parameter value
        }
    ...
    valueStr.clear();
    if (getPamModuleArgValue(pamFaillock, maxFailedAttempt, valueStr) != success)
    {
