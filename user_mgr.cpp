...
T parseUnsignedConfigValue(std::string_view valueStr)
{
    ...
    unsigned long long parsedValue =
        std::stoull(std::string(valueStr), &parsedChars, 10);
    if (parsedChars != valueStr.size())
    {
        throw std::invalid_argument("value has trailing characters");
    }
    if (parsedValue > static_cast<unsigned long long>(std::numeric_limits<T>::max()))
    {
