...
static constexpr const char* dBusObjManager = "org.freedesktop.DBus.ObjectManager";
static constexpr const char* getManagedObjectsMethod = "GetManagedObjects";
static constexpr const char* ldapMgrObjBasePath = "/xyz/openbmc_project/user/ldap";
static constexpr const char* ldapMgrInterface = "xyz.openbmc_project.User.PrivilegeMapper";
static constexpr const char* ldapMapperInterface = "xyz.openbmc_project.User.PrivilegeMapperEntry";

// Object Mapper related
static constexpr const char* objMapperService = "xyz.openbmc_project.ObjectMapper";
static constexpr const char* objMapperPath = "/xyz/openbmc_project/object_mapper";
static constexpr const char* objMapperInterface = "xyz.openbmc_project.ObjectMapper";
...
std::string getUserService(sdbusplus::bus::bus& bus,
                           const std::string& path,
                           const std::string& intf)
{
    auto mapperCall = bus.new_method_call(objMapperService,
                                          objMapperPath,
                                          objMapperInterface,
                                          getObjectMethod);
...
UserInfoMap UserMgr::getUserInfo(const std::string &userName)
{
    ...
    if (isUserExist(userName) == true)
    {
        ...
        try
        {
            auto ldapMgmtService = getUserService(bus,ldapMgrObjBasePath,ldapMgrInterface);

            auto method = bus.new_method_call(ldapMgmtService.c_str(),
                                              ldapMgrObjBasePath, dBusObjManager,
        ...
        for (auto& objpath : properties) {
            ...
            {
                if (interface.first == "xyz.openbmc_project.User.PrivilegeMapperEntry") {
                    std::string groupName;
                    std::string privilege;

                    for (const auto& property : interface.second) {
                        auto value = sdbusplus::message::variant_ns::get_if<std::string>(&property.second);
