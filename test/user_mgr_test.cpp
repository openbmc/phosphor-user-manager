...
class TestUserMgr : public testing::Test
{
  public:
                                   ...
                                   const PasswordInfo& newInfo)
    {
        ...
        const auto expirationTime = (newInfo.lastChangeDate + newInfo.maxAge) *
                                    secondsPerDay;
                                     ...
                                     const PasswordInfo& info)
    {
        EXPECT_CALL(mockManager, getShadowData(testing::StrEq(userName), _))
            .WillOnce(
                Invoke([info](auto, struct spwd& spwd) {
...
TEST_F(TestUserMgr, PasswordExpirationGet)
{
    ...
    constexpr uint64_t expirationTime = (lastChangeDate + passwordAge) * secondsPerDay;

    testPasswordExpirationGet(expiryUserName, {lastChangeDate,passwordAge}, expirationTime);
}
