...
uint64_t passwordExpirationNow()
{
    ...
    const long nextPasswordChangeInDays, PasswordExpirationInfo& info)
{
    ...
    info.lastChangeDate =
        duration_cast<days>(seconds{passwordExpirationNow()}).count() -
        lastChangeDaysAgo;
    ...
    info.passwordExpiration =
        passwordExpirationNow() + nextPasswordChangeInDays * secondsPerDay;
}
...
TEST_F(TestUserMgr, PasswordExpirationGetLastChangeZero)
{
    ...
    auto expirationTime =
        duration_cast<minutes>(seconds{passwordExpirationNow()}).count();
...
{
    uint64_t date{0};
    EXPECT_THROW(mockManager.setPasswordExpiration("unknownUser", date),
...
TEST_F(UserMgrInTest, CreateUser2)
{
    ...
    EXPECT_NO_THROW(
        UserMgr::createUser2(userName, {"redfish", "ssh"}, "priv-user", enabled,
                             info.passwordExpiration));
...
TEST_F(UserMgrInTest, CreateUser2UnexpiringPassword)
{
    ...
    EXPECT_CALL(*this, getShadowData(testing::StrEq(userName), _))
        .WillOnce(
            Invoke([&lastChangeDate, passwordAge](auto, struct spwd& spwd) {
                spwd.sp_lstchg = lastChangeDate;
                spwd.sp_max = passwordAge;
            }));
    ...
    EXPECT_CALL(*this, executeUserPasswordExpiration(
                           testing::StrEq(userName),
                           getUnexpiringPasswordLastChangeDate(), passwordAge))
        .Times(1);
