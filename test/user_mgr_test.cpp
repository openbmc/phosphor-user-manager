...
class UserMgrInTest : public testing::Test, public UserMgr
{
  public:
    ...
    MOCK_METHOD(bool, isUserExistSystem, (const std::string& userName), (override));
...
TEST_F(UserMgrInTest, CreateUserThrowsInternalFailureWhenExecuteUserAddPartiallyFails)
{
    std::string username = "whatever";
    EXPECT_CALL(*this, executeUserAdd)
        .WillOnce(testing::Throw(
...
TEST_F(UserMgrInTest, RenameUserThrowsInternalFailureIfExecuteUserModifyPartiallyFails)
{
    std::string username = "user001";
    EXPECT_NO_THROW(
        UserMgr::createUser(username, {"redfish", "ssh"}, "priv-user", true));
