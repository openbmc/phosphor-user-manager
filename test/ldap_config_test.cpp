...
class TestLDAPConfig : public testing::Test
{
  public:
    ...
    void SetUp() override
    {
        ...
        tslCacertPath = dir / fs::path("/ssl");
        mkdir(tslCacertPath.c_str(), S_IRWXU);
        tslCacertPath = tslCacertPath / fs::path("/certs");
        mkdir(tslCacertPath.c_str(), S_IRWXU);
        std::fstream fs;
        ...
        fs.close();
        fs.open(tslCacertPath / tlsCacertfile, std::fstream::out);
        fs.close();
    }
