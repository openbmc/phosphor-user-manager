...
bool changeFileOwnership(const std::string& userName)
{
    ...
    const auto configDir = std::filesystem::path(
        std::format("/home/{}/.config", userName));
    const auto mfaDir = configDir / "phosphor-user-manager";
    const auto tempSecret = mfaDir / ".google_authenticator.tmp";
